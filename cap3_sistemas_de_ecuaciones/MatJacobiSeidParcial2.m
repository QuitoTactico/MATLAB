function [s, E] = MatJacobiSeidParcial2(x0, A, b, Tol, niter, met)
    c = 0;
    error = Tol + 1;
    D = diag(diag(A));
    L = -tril(A, -1);
    ...
    % Imprime la condición inicial como Iteración 0
    ...
    while error >= Tol && c < niter
        ...
        % Ajustar c antes de imprimir y guardar el error
        c = c + 1;
    ...
    if error < Tol
        s = x0;
        fprintf('La solución aproximada es [%s] con una tolerancia de %f después de %d iteraciones\n', sprintf('%.4f ', s), Tol, c);
    else 
